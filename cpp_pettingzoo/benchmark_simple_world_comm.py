"""Benchmark C++ vs Python PettingZoo SimpleWorldComm environments."""
...
from cpp_pettingzoo.simple_world_comm.simple_world_comm import parallel_env as cpp_parallel_env
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("PettingZoo SimpleWorldComm Environment Benchmark")
    print("=" * 60)
    print(f"Configuration: {n_resets} resets, {n_steps} steps, {n_episodes} episodes")
    print(f"Max cycles per episode: {MAX_CYCLES}")
    print(
        f"Good: {NUM_GOOD}, Adversaries: {NUM_ADVERSARIES}, "
        f"Obstacles: {NUM_OBSTACLES}, Food: {NUM_FOOD}, Forests: {NUM_FORESTS}"
    )
    print()
    ...
    mpe2_factory = lambda: simple_world_comm_v3.parallel_env(
        num_good=NUM_GOOD,
        num_adversaries=NUM_ADVERSARIES,
        num_obstacles=NUM_OBSTACLES,
        num_food=NUM_FOOD,
