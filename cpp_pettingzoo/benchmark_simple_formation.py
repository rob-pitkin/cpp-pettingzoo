"""Benchmark C++ vs Python PettingZoo SimpleFormation environments."""
...
from cpp_pettingzoo.simple_formation.simple_formation import parallel_env as cpp_parallel_env
...
def benchmark_steps(env_factory, n_steps):
    ...
    for _ in range(n_steps):
        _, _, terms, truncs, _ = env.step({a: 0 for a in AGENTS})
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("PettingZoo SimpleFormation Environment Benchmark")
    print("=" * 60)
    print(f"Configuration: {n_resets} resets, {n_steps} steps, {n_episodes} episodes")
    print(f"Max cycles per episode: {MAX_CYCLES}")
    print(f"Agents: {N}, Landmarks: 1 (central)")
    print()

    cpp_factory = lambda: cpp_parallel_env(N=N, max_cycles=MAX_CYCLES)
    ...
    mpe2_factory = lambda: simple_formation_v1.parallel_env(N=N, max_cycles=MAX_CYCLES)
