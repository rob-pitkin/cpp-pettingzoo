"""SimpleLine environment with C++ backend."""
...
class parallel_env(ParallelEnv, EzPickle):
    """SimpleLine parallel environment.

    N agents cooperate to arrange themselves evenly along the line segment
    between two landmarks. Reward is the negative mean assigned distance
    (Hungarian matching), shared equally by all agents.
    """
    ...
    def __init__(
        ...
        self._cpp_env = _simple_line.SimpleLineEnv(
            N=N,
            max_cycles=max_cycles,
            continuous_actions=continuous_actions,
            dynamic_rescaling=dynamic_rescaling,
        ...
        if self.render_mode is not None:
            ...
            self.world.landmarks = [Landmark(f"landmark_{i}") for i in range(2)]
