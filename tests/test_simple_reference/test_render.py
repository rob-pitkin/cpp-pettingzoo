"""Test rendering functionality for SimpleReference environment."""
...
def test_render_shows_two_agents():
    """Test that rendering includes both agents."""
    env = parallel_env(render_mode="rgb_array")
    env.reset(seed=42)
...
def test_render_shows_landmarks():
    ...
    # (agents have colors matching goal landmarks, landmarks have RGB colors)
    # Expect at least: white background + 3 RGB landmarks + 2 agent colors = 5+ colors
